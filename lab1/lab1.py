...
def main():
    ...
    os.system(f"cp {os.path.abspath(os.path.dirname(__file__))}/* lab1")
    os.system("chmod -R u+w lab1") # Nix specific: Allow linker script to be modified.

    # Update memory region.
    replace_in_file("lab1/linker.ld", "main_ram", args.mem)

    # Compile demo
    build_path = args.build_path if os.path.isabs(args.build_path) else os.path.join("..", args.build_path)
    os.system(f"export BUILD_DIR={build_path} && {'export WITH_CXX=1 &&' if args.with_cxx else ''} cd lab1 && make")

    # Copy demo.bin
    os.system("cp lab1/lab1.bin ./")

    # Prepare flash boot image.
    python3 = sys.executable or "python3" # Nix specific: Reuse current Python executable if available.
    os.system(f"{python3} -m litex.soc.software.crcfbigen lab1.bin -o lab1.fbi --fbi --little") # FIXME: Endianness.
